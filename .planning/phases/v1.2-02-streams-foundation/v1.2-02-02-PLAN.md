...
- Store state in hidden properties: _state, _writer (null or writer reference), _controller, _queue (array of pending writes), _closeRequested, _writing
...
writable_stream.registerWritableStreamAPI(isolate, context);
```
...
3. Test app passes all 5 tests (see Task 3 verification)
